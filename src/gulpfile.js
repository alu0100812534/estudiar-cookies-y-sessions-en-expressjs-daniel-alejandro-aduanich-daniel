...
gulp.task('routing', function() {
  return gulp.src('')
    .pipe(shell.exec('node ../src/Routing.js'));
});
gulp.task('BasicRouting', function() {
  return gulp.src('')
    .pipe(shell.exec(['node ../src/Br.js']));
});
gulp.task('middle1', function() {
  return gulp.src('')
    .pipe(shell.exec(['node ../src/middleware1.js']));
});
gulp.task('middle2', function() {
  return gulp.src('')
    .pipe(shell.exec(['node ../src/middleware2.js']));
});
gulp.task('middle3', function() {
  return gulp.src('')
    .pipe(shell.exec('node ../src/middleware3.js'));
});
gulp.task('middle4', function() {
  return gulp.src('')
    .pipe(shell.exec(['node ../src/middleware4.js']));
});
